...
def get_dist_launch(args):  # some examples
    if args.dist == 'f4':
        ...
        return "CUDA_VISIBLE_DEVICES=4,5 WORLD_SIZE=2 python3 -m torch.distributed.launch --nproc_per_node=2 " \
               "--nnodes=1 --master_port={:}".format(MASTER_PORT)
...
def run(args):
    ...
    if args.task == 'itr_cuhk':
        ...
        assert os.path.exists("PATH_TO_pa100k")
