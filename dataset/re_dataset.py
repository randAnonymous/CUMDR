...
class re_train_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.eda:
            gpt = pre_caption(ann['gpt'], self.max_words, self.icfg_rstp, True, self.eda_p)
            caption1 = pre_caption(ann['caption'], self.max_words, self.icfg_rstp, True, self.eda_p)
            return image1, caption, caption1, self.img_ids[ann['image_id']], gpt
        elif self.attr:
...
class re_test_dataset(Dataset):
    def __init__(self, ann_file, config, transform):
        ...
        for img_id, ann in enumerate(self.ann):
            ...
            self.img2txt[img_id] = []
            onegpt = pre_caption(ann['gpt'], self.max_words, self.icfg_rstp)
            self.gpt.append(onegpt)
            self.gpt.append(onegpt)
            t = 0
...
class re_test_dataset_icfg(Dataset):
    def __init__(self, config, transform):
        ...
        for img_id, ann in enumerate(self.ann):
            ...
            onegpt = pre_caption(ann['gpt'], self.max_words, icfg_rstp=True)
            self.gpt.append(onegpt)
            # self.gpt.append(onegpt)
