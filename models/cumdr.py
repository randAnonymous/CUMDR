...
from .uncertainty_aware import DisTrans
from .uncertainty_aware import gaussian_modeling
from .coAttention import coAttenTransformer
...
class CUMDR(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        if config['diffusion']:
            self.diffusion_model = Diffusion(config['embed_dim'])
        if config['uncertainty']:
            self.img_gaussian = DisTrans(dim=config['embed_dim'], indim=1024, num_heads=8)
            self.text_gaussian = DisTrans(dim=config['embed_dim'], indim=768, num_heads=8)
            self.text_mlm_gaussian = DisTrans(dim=768, indim=768, num_heads=8)
            self.cross_gaussian = DisTrans(dim=config['embed_dim'], indim=256, num_heads=8)
            self.uncertain_image_proj = nn.Linear(in_features=1024, out_features=config['embed_dim'])
            self.uncertain_text_proj = nn.Linear(in_features=768, out_features=config['embed_dim'])
            self.uncertrain_cross_proj = nn.Linear(in_features=768, out_features=config['embed_dim'])
            self.d_itm_head = build_mlp(input_dim=config['embed_dim'], output_dim=2)
        if config['fine_grained']:
            self.fine_grained_proj_txt = nn.Linear(768, config['embed_dim'])
            self.fine_grained_proj_img = nn.Linear(self.vision_width, config['embed_dim'])
            self.fine_grained_proj_txt2 = nn.Linear(config['embed_dim'], 1)
            self.fine_grained_proj_img2 = nn.Linear(config['embed_dim'], 1)
            self.norm_itm = nn.BatchNorm1d(config['batch_size_train'])

        if config['gpt']:
            self.img_caption = coAttenTransformer(config['embed_dim'], 8, 8)
            self.gpt_caption = coAttenTransformer(config['embed_dim'], 8, 8)
    ...
    def build_masked_image(self, image, masks):
        ...
        masks = masks.cuda()
        B, C, H, W = image.shape
        mask_tokens = self.mask_token.repeat(B, 1, 12, 4) #B*3*32*32 -> B*3*384*128
        temp_mask = masks.reshape(B, 12, 4).unsqueeze(1).repeat(1, 3, 32, 32)
        # mask_tokens = self.mask_token.expand(B, L, -1)
        # tmp_mask = mask.unsqueeze(1).repeat(1, L, 1)
        x = torch.mul(image, (1.0 - temp_mask)) + torch.mul(temp_mask, mask_tokens)
        # x = x.reshape(B, 3, args.img_size[0], args.img_size[1])
        return x
    ...
    def get_unc_loss(self, image_fetures, text_fetures, pid, epoch, total_epoch, amplititude=0.7, image_id=None):
        """
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None, uncertainty=False):
        """
        ...
        if uncertainty:
            cross_pos = self.uncertrain_cross_proj(cross_pos)
            cross_neg = self.uncertrain_cross_proj(cross_neg)
            cross_pos, cross_neg, _, _, _, _ = gaussian_modeling(image_embeds=cross_pos, extend_image_masks=None,
                                                                text_embeds=cross_neg, extend_text_masks=None,
                                                                img_gau_encoder=self.cross_gaussian, 
                                                                txt_gau_encoder=self.cross_gaussian,
                                                                mu_num=1, sample_num=5)
            cross_pos = cross_pos.reshape(6, -1, cross_pos.shape[-2], cross_pos.shape[-1])[:, :, 0, :]
            cross_neg = cross_neg.reshape(6, -1, cross_neg.shape[-2], cross_neg.shape[-1])[:, :, 0, :]
            itm_labels = torch.cat([torch.ones(bs, dtype=torch.long),
                                    torch.zeros(2 * bs, dtype=torch.long)], dim=0).to(image_embeds.device)
            itm_loss = 0
            for i in range(6):
                output = self.d_itm_head(torch.cat([cross_pos[i], cross_neg[i]], dim=0))
                itm_loss = itm_loss + F.cross_entropy(output, itm_labels)
            return itm_loss / 6
    ...
    def Wasserstein2(self, mu1, sigma1, mu2, sigma2): 
        bs1 = mu1.shape[0]
        bs2 = mu2.shape[0]
    ...
    def get_irtr_loss(self, img_mu, img_sigma, txt_mu, txt_sigma, idx = None):
        w2 = self.Wasserstein2(img_mu, img_sigma, txt_mu, txt_sigma)[0]
        if idx != None:
            label = idx
        label = torch.arange(img_sigma.shape[0], device=img_mu.device)
        similarity = -0.005 * w2 + 6
        conloss = F.cross_entropy(similarity, label)
        return conloss

    def get_fine_grained_loss(self, sim):
        logits = torch.arange(0, sim.shape[0], dtype=torch.long, device = sim.device)
        sim = F.softmax(sim)
        return F.cross_entropy(sim, logits)
    
    def get_dvlc_loss(self, img_sigma, text_sigma, img_mu, text_mu):
        img_mu = img_mu[:, 0]
        img_sigma = torch.exp(img_sigma[:, 0])
        text_mu = text_mu[:, 0]
        text_sigma = torch.exp(text_sigma[:, 0])
        bs = img_mu.shape[0]
        W2_distance, mu_distance = self.Wasserstein2(img_mu, img_sigma, text_mu, text_sigma)
        similarity = (-0.005 * W2_distance + 4)
    ...
    def margin_entropy_loss(self, margin, logsigma): # 见PUM和ReID那两篇文章
    ...
    def get_dmlm_loss(self, mlm_embeds, mlm_labels, image_logsigma, text_logsigma, num_sample, masked_pos):
        mlm_embeds = mlm_embeds.reshape(num_sample+1, -1, mlm_embeds.shape[-2],mlm_embeds.shape[-1])
        mlm_loss = []
        for i in range(num_sample+1):
            temp = self.text_encoder.gather_seq_out_by_pos(mlm_embeds[i], masked_pos)
            mlm_logits = self.text_encoder.cls(temp)
            mlm_loss.append(F.cross_entropy(mlm_logits.view(-1, 30522), 
                                            mlm_labels.view(-1), ignore_index=-100)) # 30522 is vocab size
        mlm_loss = sum(mlm_loss) / (num_sample + 1)
        mlm_logits = torch.mean(mlm_logits, dim=0)
        margin_loss1 = self.margin_entropy_loss(300, image_logsigma)
        margin_loss2 = self.margin_entropy_loss(300, text_logsigma)
        margin_loss = (margin_loss1 + margin_loss2) / 2
        return margin_loss + mlm_loss

    def get_mlm_embeds(self, text_ids_masked, text_atts, image_embeds, image_atts):
        mlm_embeds = self.text_encoder.bert(text_ids_masked,
                                 attention_mask=text_atts,
                                 encoder_hidden_states=image_embeds,
                                 encoder_attention_mask=image_atts,
                                 return_dict=True,
