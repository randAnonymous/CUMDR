...
from models.model_retrieval import CUMDR_Retrieval
...
from reTools import evaluation, mAP, newmAP
...
def main(args, config):
    ...
    model = CUMDR_Retrieval(config=config)
    ...
    if args.evaluate:
        ...
        test_loader = create_loader([test_dataset], [None],
                                    ...
                                    is_trains=[False],
                                    collate_fns=[None])[0]
        score_test_t2i, masked_sim = evaluation(model_without_ddp, test_loader,
                                                    tokenizer, device, config, args)

        if utils.is_main_process():
            print('test_result:', flush=True)
            mAP(score_test_t2i, test_loader.dataset.g_pids, test_loader.dataset.q_pids, dataloader=test_loader)
            _ = newmAP(masked_sim, test_loader.dataset.g_pids, test_loader.dataset.q_pids)
        ...
        for epoch in range(0, max_epoch):
            ...
            if (epoch + 1) % 1 == 0:
                ...
                score_test_t2i, masked_sim = evaluation(model_without_ddp, test_loader,
                                                            tokenizer, device, config, args, diffusion=diffusion)

            if utils.is_main_process():
                test_result = mAP(score_test_t2i, test_loader.dataset.g_pids, test_loader.dataset.q_pids, test_loader)
                _ = newmAP(masked_sim, test_loader.dataset.g_pids, test_loader.dataset.q_pids)
